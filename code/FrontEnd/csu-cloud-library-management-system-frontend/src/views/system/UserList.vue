<template>
  <div class="user-list-container">
    <el-card class="common-card">
      <div class="toolbar">
        <el-input v-model="searchKeyword" placeholder="搜索账号/邮箱..." style="width: 300px;" clearable
          @clear="handleSearch">
          <template #append>
      ...
      <el-table :data="users" v-loading="loading" style="width: 100%; margin-top: 20px;">
        ...
        <el-table-column prop="role" label="角色">
          <template #default="{ row }">
            <el-tag :type="row.role === 'ADMIN' ? 'danger' : 'info'">{{ row.role === 'ADMIN' ? '管理员' : '普通用户'
              }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="150">
          <template #default="{ row }">
            <el-button v-if="row.role !== 'ADMIN'" :type="row.isDeleted === 1 ? 'success' : 'danger'" size="small"
      ...
      <div class="pagination">
        <el-pagination layout="total, prev, pager, next" :total="total" v-model:current-page="currentPage"
          v-model:page-size="pageSize" @current-change="fetchUsers" />
      </div>
    </el-card>
  </div>
</template>
